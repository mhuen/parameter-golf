...
class GPT(nn.Module):
    def __init__(self, vocab_size, num_layers, model_dim, num_heads, num_kv_heads, mlp_mult,
        ...
        self.shared_block = SharedBlock(model_dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init)

        # Per-layer scalars (unique per application).
        self.layer_scalars = nn.ModuleList([LayerScalars(model_dim) for _ in range(num_layers)])
    ...
    def forward(self, input_ids, target_ids, lora=None):
        ...
        for i in range(self.num_encoder_layers):
            ...
            x = self.shared_block(x, ls.attn_scale, ls.mlp_scale, qd, vd)
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            mix = ls.resid_mix.to(dtype=x.dtype)
            x = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
            qd = lora.q_loras[self.num_encoder_layers + i] if lora else None
            vd = lora.v_loras[self.num_encoder_layers + i] if lora else None
            x = self.shared_block(x, ls.attn_scale, ls.mlp_scale, qd, vd)
        x = self.final_norm(x)
        if self.tie_embeddings:
...
class BatchedTTTLoRA(nn.Module):
    """LoRA adapters per layer application — all reference shared_block's q/v dims."""
    def __init__(self, bsz, model, rank):
        ...
        q_out = model.shared_block.attn.c_q.weight.shape[0]
        v_out = model.shared_block.attn.c_v.weight.shape[0]
...
def main():
    ...
    # Optimizer: shared_block 2D -> Muon, layer_scalars + skip_weights + shared_block 1D -> Adam
    shared_block_named = list(base_model.shared_block.named_parameters())
    matrix_params = [p for n, p in shared_block_named if p.ndim == 2 and not any(pat in n for pat in CONTROL_TENSOR_NAME_PATTERNS)]
    scalar_params = [p for n, p in shared_block_named if p.ndim < 2 or any(pat in n for pat in CONTROL_TENSOR_NAME_PATTERNS)]
    ...
    log0(f"model_params:{n_params} num_layers:{args.num_layers} model_dim:{args.model_dim} shared_block:True")
