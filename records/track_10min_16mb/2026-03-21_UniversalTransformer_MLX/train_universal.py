...
class BatchedTTTLoRA(nn.Module):
    ...
    def __init__(self, bsz, model, rank):
        ...
        q_out = model.shared_blocks[0].attn.c_q.weight.shape[0]
        v_out = model.shared_blocks[0].attn.c_v.weight.shape[0]
...
def eval_val_ttt_lora(
    ...
    chunk_size, eval_seq_len, batch_size, lora_rank = (
        args.ttt_chunk_size,
    ...
    return float(loss_sum.item() / token_count.item()), float(
        (loss_sum.item() / math.log(2.0)) / byte_sum.item()
    )
...
def main():
    ...
    if master_process:
        with open(f"{model_dir}/final_model_{args.run_id}.int8.ptz", "wb") as f:
            f.write(quant_blob)
        qfb = os.path.getsize(f"{model_dir}/final_model_{args.run_id}.int8.ptz")
    ...
    torch.cuda.synchronize()
    log0(
        f"final_int8_ttt_lora val_loss:{ttt_val_loss:.4f} val_bpb:{ttt_val_bpb:.4f} eval_time:{1000.0 * (time.perf_counter() - t_ttt):.0f}ms"
    )
