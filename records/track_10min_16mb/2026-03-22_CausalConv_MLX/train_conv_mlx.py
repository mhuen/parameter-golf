#!/usr/bin/env python3
"""
Convolutional Language Model (MLX) — replaces transformer attention blocks with
causal depthwise-separable 1D convolutions of increasing kernel sizes (3..31).

Architecture:
  - Token embedding + RMSNorm → residual stream
  - Each block: depthwise causal conv (spatial mixing) + pointwise MLP (channel mixing)
  - Both sub-blocks write to the residual stream with learned scales
  - Encoder/decoder halves with skip connections (same as baseline GPT)
  - Tied embeddings for LM head, logit softcap
"""
...
# - 23 conv blocks at width 512: 3 layers with kernel_size=3, then 20 layers with kernel_size=5
# - depthwise causal conv + relu^2 MLP with 2x expansion
...
class Hyperparameters:
    ...
    run_id: str = f"{os.environ.get('RUN_ID', str(uuid.uuid4()))}-{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}"
    ...
    num_layers: int = int(os.environ.get("NUM_LAYERS", 23))
    ...
    # Conv-specific: expansion ratio for the intermediate depthwise conv channels
    conv_expand: int = int(os.environ.get("CONV_EXPAND", 1))
...
class CausalDepthwiseConv(nn.Module):
    ...
    Causality is enforced by left-padding the input by (kernel_size - 1) so that
    ...
    def __init__(self, dim: int, kernel_size: int):
        ...
        # For depthwise (groups=dim): (dim, kernel_size, 1)
        scale = 1.0 / math.sqrt(kernel_size)
        self.weight = mx.random.normal((dim, kernel_size, 1)) * scale

    def __call__(self, x: mx.array) -> mx.array:
        # x: (B, T, D) — MLX conv1d expects (B, T, C_in)
        bsz, _, dim = x.shape
        # Causal left-padding
        if self.kernel_size > 1:
            pad = mx.zeros((bsz, self.kernel_size - 1, dim), dtype=x.dtype)
            x = mx.concatenate([pad, x], axis=1)
        # MLX conv1d: input (B, T, C_in), weight (C_out, K, C_in/groups)
        y = mx.conv1d(x, self.weight.astype(x.dtype), groups=self.dim)
        return y


class MLP(nn.Module):
    """Pointwise MLP with relu^2 activation (same as baseline)."""
...
class ConvBlock(nn.Module):
    """Convolutional block that writes to the residual stream.

    Sub-block 1: RMSNorm → CausalDepthwiseConv → scale → residual add
    Sub-block 2: RMSNorm → MLP (pointwise, relu^2) → scale → residual add

    Maintains the same resid_mix with x0 as the baseline transformer block.
    """

    def __init__(self, dim: int, kernel_size: int, mlp_mult: int):
        super().__init__()
        self.conv_norm = RMSNormNoWeight()
    ...
    def __call__(self, x: mx.array, x0: mx.array) -> mx.array:
        # Mix current residual with original embedding (like baseline)
        mix = self.resid_mix.astype(x.dtype)
        x = mix[0][None, None, :] * x + mix[1][None, None, :] * x0

        # Depthwise causal conv (spatial/temporal mixing → replaces attention)
        conv_out = self.conv(self.conv_norm(x))
        x = x + self.conv_scale.astype(x.dtype)[None, None, :] * conv_out

        # Pointwise MLP (channel mixing)
...
class ConvLM(nn.Module):
    """Convolutional language model with the same encoder/decoder skip structure as baseline GPT.
    ...
    def __init__(
        ...
        if len(kernel_sizes) != num_layers:
            raise ValueError(
                f"Need {num_layers} kernel sizes, got {len(kernel_sizes)}: {kernel_sizes}"
            )
        ...
        self.blocks = [
            ConvBlock(dim, kernel_sizes[i], mlp_mult) for i in range(num_layers)
        ]
        self.final_norm = RMSNormNoWeight()

        # Zero-init output projections (conv depthwise weights are already small-init)
...
class SplitOptimizers:
    def __init__(self, model: ConvLM, args: Hyperparameters):
        self.args = args
    ...
    def step(self, model: ConvLM, grads_tree: dict, step: int, lr_mul: float) -> None:
        params = dict(tree_flatten(model.parameters()))
...
def main() -> None:
    ...
    # Resolve kernel sizes — pad or truncate to match num_layers
    kernel_sizes = KERNEL_SIZES[: args.num_layers]
    if len(kernel_sizes) < args.num_layers:
        # Repeat the last kernel size if not enough specified
    ...
    log(f"architecture:convolutional kernel_sizes:{kernel_sizes}")
    ...
    log(
        ...
        f"dim:{args.model_dim} mlp_mult:{args.mlp_mult} "
    ...
    log(
        ...
        f"conv_weight:{model.blocks[0].conv.weight.dtype} "
