"""
Shared-MLP Deep Attention GPT — PyTorch implementation.
...
# -----------------------------
# TRANSFORMER MODULES (Shared MLP)
# -----------------------------
...
class MLP(nn.Module):
    # relu^2 MLP — shared across all blocks
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, x0: Tensor, mlp: MLP, q_delta_fn=None, v_delta_fn=None) -> Tensor:
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, lora=None) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            x = self.blocks[i](x, x0, self.shared_mlp, qd, vd)
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            x = self.blocks[bi](x, x0, self.shared_mlp, qd, vd)
        x = self.final_norm(x)
        if self.tie_embeddings:
...
def main() -> None:
    ...
    # Optimizer split: shared_mlp weights go to Muon along with block attention weights
    block_named_params = list(base_model.blocks.named_parameters())
    shared_mlp_named_params = [("shared_mlp." + n, p) for n, p in base_model.shared_mlp.named_parameters()]
    matrix_params = [
        p
        for name, p in block_named_params + shared_mlp_named_params
    ...
    log0(
        f"num_layers:{args.num_layers} model_dim:{args.model_dim} mlp_mult:{args.mlp_mult} shared_mlp:True "
