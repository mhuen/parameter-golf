"""
Analytical Scaling (No RMS Norm) — record variant of train_gpt.py
...
maintain variance ≈ 1 and mean ≈ 0 throughout the forward pass:
  ...
  - SwiGLU: silu(gate)*up for gate,up~N(0,1) has 2nd moment ≈ 0.3554 → scale by 1/√0.3554.
...
# -----------------------------
# TRANSFORMER MODULES (Analytical Scaling — No RMSNorm)
# -----------------------------
...
class CastedLinear(nn.Module):
    ...
    # Weights kept in fp32 for optimizer quality, cast at matmul time.
    def __init__(self, in_dim: int, out_dim: int):
        super().__init__()
        self.weight = nn.Parameter(torch.randn(out_dim, in_dim, dtype=torch.float32))
        self.in_scale = in_dim ** -0.5

    def forward(self, x: Tensor) -> Tensor:
        return F.linear(x, self.weight.to(x.dtype)) * self.in_scale
...
class MLP(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        return self.proj(F.silu(self.gate_proj(x)) * self.up_proj(x) * SWIGLU_SCALE)
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, x0: Tensor, q_delta_fn=None, v_delta_fn=None) -> Tensor:
        ...
        qd = q_delta_fn(x) if q_delta_fn is not None else None
        vd = v_delta_fn(x) if v_delta_fn is not None else None
        attn_out = self.attn(x, qd, vd)
        # Residual add: two signals → var doubles → scale by 1/√2.
        x = (x + self.attn_scale.to(dtype=x.dtype)[None, None, :] * attn_out) * INV_SQRT2
        x = (x + self.mlp_scale.to(dtype=x.dtype)[None, None, :] * self.mlp(x)) * INV_SQRT2
        return x
...
class GPT(nn.Module):
    def __init__(
        ...
        self.lm_head_scale = model_dim ** -0.5  # LM head matmul reduces over dim
        ...
        self.final_gain = nn.Parameter(torch.ones(model_dim, dtype=torch.float32))
    ...
    def _init_weights(self) -> None:
        ...
        # Zero-init output projections for attn and mlp (residual stream starts clean).
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, lora=None) -> Tensor:
        ...
        for i in range(self.num_decoder_layers):
            ...
            if skips:
                ...
                x = (x + self.skip_weights[i].to(dtype=x.dtype)[None, None, :] * skips.pop()) * INV_SQRT2
            qd = lora.q_loras[bi] if lora else None
            vd = lora.v_loras[bi] if lora else None
            x = self.blocks[bi](x, x0, qd, vd)
        # Learnable per-dim gain replaces adaptive final RMS norm.
        x = x * self.final_gain.to(dtype=x.dtype)[None, None, :]
        if self.tie_embeddings:
            # LM head matmul reduces over dim → scale by 1/√dim.
            logits = F.linear(x, self.tok_emb.weight) * self.lm_head_scale
        else:
...
def main() -> None:
    ...
    # final_gain is a top-level parameter, add it to scalar params.
    scalar_params.append(base_model.final_gain)
