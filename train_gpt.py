...
try:
    ...
    _GRAM_NS_LIB = False
    print(
        "Could not import optimized Gram Newton-Schulz from Dao-AILab; falling back to pure PyTorch implementation. To use the optimized version, ensure you have CUDA 12.9+ and a compatible GPU, and install the gram_newton_schulz package from Dao-AILab."
    )
...
def zeropower_via_newtonschulz5(
    ...
    # When gram_ns=True and the matrix is rectangular, use the stabilized Gram
    # Newton-Schulz variant which iterates on the smaller Gram matrix.
...
def build_cu_seqlens(input_ids: Tensor, bos_id: int, pad_to: int = 0) -> tuple[Tensor, int]:
    ...
    # Enforce batch-element starts as document boundaries too
    ...
    if pad_to > 0 and cu_seqlens.size(0) < pad_to:
        # Pad with terminal value → zero-length segments the kernel skips.
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, doc_mask=None, cu_seqlens=None, max_seqlen=None) -> Tensor:
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(
            dim, num_heads, num_kv_heads, rope_base, qk_gain_init, use_fa4=use_fa4
        )
    ...
    def forward(self, x: Tensor, x0: Tensor, doc_mask=None, cu_seqlens=None, max_seqlen=None) -> Tensor:
        mix = self.resid_mix.to(dtype=x.dtype)
        x = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        attn_out = self.attn(
            self.attn_norm(x), doc_mask=doc_mask,
            cu_seqlens=cu_seqlens, max_seqlen=max_seqlen,
        )
...
class GPT(nn.Module):
    ...
    def forward(
        ...
        doc_mask=None, cu_seqlens=None, max_seqlen=None,
    ) -> Tensor:
...
def main() -> None:
    ...
    # Fixed cu_seqlens budget for torch.compile (assumes min doc len ~64 tokens).
