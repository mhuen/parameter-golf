...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, x0: Tensor, doc_mask=None) -> Tensor:
        mix = self.resid_mix.to(dtype=x.dtype)
        x = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        attn_out = self.attn(self.attn_norm(x), doc_mask=doc_mask)
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, doc_mask=None) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            x = self.blocks[i](x, x0, doc_mask=doc_mask)
            skips.append(x)
